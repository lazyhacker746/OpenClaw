...
import { Search, Download, ShieldAlert, Zap } from 'lucide-react';
    ...
    <div className="bg-white/80 dark:bg-gray-900/40 backdrop-blur-xl border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-xl flex-1 flex flex-col min-h-[600px] transition-colors duration-300">
      <div className="flex justify-between items-center mb-6 border-b border-gray-200 dark:border-gray-800 pb-4">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-lg font-semibold text-gray-800 dark:text-white tracking-wide">Acquired Intelligence</h2>
        </div>
        {results.length > 0 && (
      ...
      {error && (
        <div className="bg-red-100 dark:bg-red-950/30 border border-red-300 dark:border-red-500/50 rounded-lg p-4 flex items-start space-x-3 text-red-700 dark:text-red-400 mb-6">
          ...
          <p className="text-sm font-mono font-semibold">{error}</p>
        </div>
      )}
      ...
      {!loading && results.length === 0 && !error && (
        <div className="flex-1 flex flex-col items-center justify-center text-gray-400 dark:text-gray-600">
          ...
          <p className="font-mono text-sm tracking-widest uppercase font-semibold">Awaiting Parameters</p>
        </div>
      )}
      ...
      {results.length > 0 && (
        <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-800 custom-scrollbar">
          <table className="w-full text-sm text-left text-gray-700 dark:text-gray-300">
            <thead className="text-xs font-mono uppercase bg-gray-50 dark:bg-black/60 text-purple-700 dark:text-purple-400/80 font-bold border-b border-gray-200 dark:border-gray-800">
              <tr>
                <th className="px-5 py-4 whitespace-nowrap">Target Identity</th>
                <th className="px-5 py-4">Intel (Reviews)</th>
                <th className="px-5 py-4">Vulnerability (Web)</th>
                <th className="px-5 py-4">Comms Link</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800/50">
              {results.map((lead, idx) => (
                <tr key={idx} className="bg-white dark:bg-black/20 hover:bg-purple-50 dark:hover:bg-purple-900/10 transition-colors group">
                  ...
                  <td className="px-5 py-4 font-mono font-bold text-purple-600 dark:text-purple-400">{lead['Review Count']}</td>
                  <td className="px-5 py-4">
                    {lead['Website Faults'] !== 'N/A' ? (
                      ...
                      <span className="text-emerald-600 dark:text-emerald-400 text-xs bg-emerald-100 dark:bg-emerald-400/10 border border-emerald-200 dark:border-emerald-400/20 px-2 py-1 rounded-md font-semibold">No Infrastructure</span>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    <a href={lead['WhatsApp Link']} target="_blank" rel="noreferrer" 
                      className="inline-flex items-center space-x-1 text-purple-600 dark:text-purple-500 hover:text-purple-800 dark:hover:text-purple-300 font-bold transition-colors">
                      <span>Execute Pitch</span>
